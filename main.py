...
# Set up display and game settings
...
if FULLSCREEN:
    ...
    SCREEN_SIZE = (display_info.current_w, display_info.current_h)
    screen = pygame.display.set_mode(SCREEN_SIZE)  # Fixed typo from `sreen` to `screen`

# Set up scenes
SCENE_MAIN_MENU = MainMenu(SCREEN_SIZE, screen)
SCENE_GAME = None 
SCENE_GAME = Pause(SCREEN_SIZE, screen)  # Game paused scene
SCENE_GAME_OVER = GameOver(SCREEN_SIZE, screen)
SCENE_PAUSE = Pause(SCREEN_SIZE, screen)  # This line was missing
...
while not done:
    ...
    screen.fill(BACKGROUND_COLOUR)  # Fixed typo `screen.feel` to `screen.fill`
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.next_state == STATE_PRE_WAVE:
                ...
                SCENE_GAME.pause_button.background_colour = BUTTON_DISABLED_COLOUR
                current_scene = SCENE_PAUSE  # Now we can switch to SCENE_PAUSE
